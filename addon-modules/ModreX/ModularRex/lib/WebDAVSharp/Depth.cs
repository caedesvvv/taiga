...
namespace WebDAVSharp
{
    ...
    {
        public static DepthHeader ParseDepth(IHttpRequest request)
        {
            ...
            if (depths == null)
            {
                ...
                throw new BadRequestException("No depth header found");
            }
            if (depths.Length == 1)
            {
                ...
                else
                {
                    ...
                    throw new BadRequestException("Unknown value in depth header");
                }
            }
            else if (depths.Length == 2)
            {
                ...
                else
                {
                    ...
                    throw new BadRequestException("Unknown values in depth header");
                }
            }
            else
            {
                //something awful has happened. depth should not contain more than two values
                throw new BadRequestException("More than two arquments in Depth header");
            }
        }
    }
}
