...
namespace OgreSceneImporter
{
    public class OgreSceneImportModule : IRegionModule
    {
	    ...
	    m_log.Info("[OGRESCENE]: Loaded scene file: " + fileName + ".scene");
	    ...
	    m_log.Info("[OGRESCENE]: Loading material file: " + fileName + ".material");
        ...
        private void AddObjectsToScene(SceneNode node, Dictionary<string, UUID> materials, string additionalSearchPath)
        {
            ...
            if (node.Entities.Count >= 0) //add this to scene and do stuff
            {
                ...
                {
                    m_log.InfoFormat("[OGRESCENE]: About to insert object {0}", ent.MeshName);
                    ...
                    {
                        m_log.InfoFormat("[OGRESCENE]: Inserting object {0}", ent.MeshName);
                        if (objPos.Z < 20)
